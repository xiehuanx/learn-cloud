...
public class SysUserController extends BaseController<ISysUserService, SysUser, Long> {
    ...
    @ApiOperation(value = "修改当前登录用户密码接口", notes = "修改当前登录用户密码接口", produces = "application/json")
    @PutMapping("/updateUserPwd")
    public R<Boolean> updateUserPwd(@RequestBody SysUser sysUser) {
        UserDetailsDto userDetails = SecurityUtils.getUserDetails();
        sysUser.setId(userDetails.getId());
        sysUser.setUsername(userDetails.getUsername());
        return R.ok(iSysUserService.updateUserPwd(sysUser));
    }
